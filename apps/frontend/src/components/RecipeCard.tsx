    ...
    >
      <div className="bg-gradient-to-b from-blue-100 to-white rounded-xl shadow-lg hover:scale-105 transition-all ease-in-out flex flex-col h-[430px] w-[300px]">
        ...
        <div className="relative h-[200px] w-full">
          ...
          <button
            onClick={handleLike}
            className={`absolute top-3 right-3 z-10 w-8 h-8 flex items-center justify-center p-2 rounded-full transition-all duration-300 ${
              localLiked ? "bg-red-500 text-white" : "bg-gray-200"
            } shadow`}
          >
            <Heart
        ...
        <div className="p-6 flex flex-col h-[230px]">
          <h2
            className="text-xl font-semibold text-black mb-2 text-center hover:text-blue-500 transition-colors duration-300 
                       line-clamp-2 overflow-hidden h-[60px]" 
          >
            {recipe.title}
          </h2>

          <p
            className="text-black text-center text-sm line-clamp-3 mb-2 overflow-hidden h-[62px]" 
          >
            {recipe.description}
          </p>

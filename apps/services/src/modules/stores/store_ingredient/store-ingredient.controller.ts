...
export class StoreIngredientController {
  ...
  @ApiResponse({ status: 201, description: 'Ingredient created successfully', type: StoreIngredient })
  @ApiResponse({ status: 404, description: 'Store not found' })
  async create(@Body() createStoreIngredientDto: CreateStoreIngredientDto): Promise<StoreIngredient> {
    return this.storeIngredientService.create(createStoreIngredientDto);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get a store ingredient by ID' })
  @ApiResponse({ status: 200, description: 'The ingredient found', type: StoreIngredient })
  @ApiResponse({ status: 404, description: 'Ingredient not found' })
  async findOne(@Param('id', ParseIntPipe) id: number): Promise<StoreIngredient> {
    return this.storeIngredientService.findOne(+id);
  }

  @Put(':id')
  @ApiOperation({ summary: 'Update a store ingredient by ID' })
  @ApiResponse({ status: 200, description: 'Ingredient updated successfully', type: StoreIngredient })
  async update(
